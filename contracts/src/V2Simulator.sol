...
contract V2Simulator is IFlashLoanRecipient, IUniswapV2Callee {
    // can perform flashloan, multihop swaps in Uniswap V2 variant pools
    using SafeERC20 for IERC20;

    address public owner;
    IWETH public mainCurrency;
    ...
    function setMainCurrency(address _mainCurrency) external {
    ...
    function _execute(bytes memory data) internal returns (uint amountOut) {
        ...
        for (uint8 i; i < nhop; ) {
            ...
            inputToken.transfer(pair, amountOut);

            uint256 reserveIn;
            uint256 reserveOut;

            {
                (uint256 reserve0, uint256 reserve1, ) = pairContract
                    .getReserves();
            ...
            uint256 realAmountIn = inputToken.balanceOf(pair) - reserveIn;
            amountOut = _getAmountOut(realAmountIn, reserveIn, reserveOut);
